...
const AddForm = () => {
  ...
  const handleSubmit = e => {
        ...
        setTagList([])
        setIngredientList([])
        // let tmpData =[...data.content,objReceipt]
        ...
        <div style={{display:'flex',justifyContent:'space-between'}} className="tags">
            {
              ...
              readTagList.map((tag, i) => (
                <button type='button' id={i} value={tag} onClick={(e)=>tagHandling(e.target)} style={{width:'70px',fontSize:'10px',color:'white',backgroundColor:'green',display:'flex',justifyContent:'space-evenly'}}>
                  {tag}
                </button>
            ))
